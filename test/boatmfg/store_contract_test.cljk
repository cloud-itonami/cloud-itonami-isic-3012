(ns boatmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `boatmfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (false? (:verified? (store/equipment-unit s "watertest-002"))))
    (is (false? (:registered? (store/equipment-unit s "watertest-002"))))
    (is (= ["molder-001" "watertest-002"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-category :motorboat}})
    (is (= :motorboat (:product-category (store/batch s "batch-001"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "watertest-002" :severity :high}})
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-quantity
  (let [s (seeded)]
    ...
    (is (= 15.0 (:shipped-units (store/batch s "batch-001")))
        "10.0 seeded + 5.0 committed")))

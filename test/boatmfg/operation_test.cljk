(ns boatmfg.operation-test
  "Smoke tests for the compiled BoatOperationActor graph itself (build
  + one happy path per op). The governor's full rule contract (HARD
  holds, escalation, phase gating) is exercised in
  `boatmfg.governor-contract-test`; the Store contract in
  `boatmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "BoatOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "molder-001" :severity :moderate :description "hull lamination anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))

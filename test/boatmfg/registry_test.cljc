(ns boatmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [boatmfg.registry :as r]))
...
(deftest small-shipment-within-quantity-does-not-exceed
  (is (false? (r/shipment-quantity-exceeded?
               {:quantity-units 40.0 :shipped-units 10.0} 5.0))))
...
(deftest known-product-categories-are-valid
  (doseq [c [:sailboat :motorboat :inflatable-boat :yacht :catamaran
...
(deftest fabricated-product-category-is-invalid
  (is (not (r/product-category-valid? :hovercraft)))
  (is (not (r/product-category-valid? nil))))

;; ----------------------------- hull-length-valid? -----------------------------

(deftest typical-hull-length-is-valid
  (is (r/hull-length-valid? 0))
  (is (r/hull-length-valid? 3.5))
  (is (r/hull-length-valid? 12.5))
  (is (r/hull-length-valid? 100)))

(deftest negative-hull-length-is-invalid
  (is (not (r/hull-length-valid? -1))))

(deftest excessive-hull-length-is-invalid
  (is (not (r/hull-length-valid? 9999.0)))
  (is (not (r/hull-length-valid? 101))))

(deftest non-numeric-or-missing-hull-length-is-invalid
  (is (not (r/hull-length-valid? nil)))
  (is (not (r/hull-length-valid? "12.5"))))

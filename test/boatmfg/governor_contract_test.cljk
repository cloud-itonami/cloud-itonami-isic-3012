(ns boatmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control hull-molding/rigging/final-
  assembly/water-test equipment directly... does NOT authorize or
  execute maintenance/equipment operations... does NOT self-issue an
  ISO 12217 / CE Recreational Craft Directive marine-classification
  certification') implemented faithfully. The single invariant under
  test:

    BoatAdvisor never schedules maintenance, flags a safety concern,
    or coordinates a shipment the Boat Building Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :sailboat}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :sailboat (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the ISO 12217/CE RCD marine-classification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-product-category-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-category :hovercraft}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-category} (-> (store/ledger db) last :basis)))
    (is (not= :hovercraft (:product-category (store/batch db "batch-001"))) "fabricated product-category never lands in the SSoT")))

(deftest invalid-hull-length-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:hull-length-m 9999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-hull-length} (-> (store/ledger db) last :basis)))
    (is (not= 9999.0 (:hull-length-m (store/batch db "batch-001"))) "fabricated hull-length never lands in the SSoT")))

(deftest invalid-defect-rate-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:hull-defect-rate-percent 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-defect-rate} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:hull-defect-rate-percent (store/batch db "batch-001"))) "fabricated hull-defect-rate never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "molder-001" :severity :moderate
                                            :description "hull lamination void, buoyancy test value variance"}}
                       coordinator)]
      (is (= :interrupted (:status res)))

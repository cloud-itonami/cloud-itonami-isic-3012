(ns boatmfg.store
  "SSoT for the pleasure-and-sporting-boat-building plant-operations
  ...
  Scope note: like its closest sibling (`cloud-itonami-isic-3092`'s
  own `bikemfg.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external boat-building capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-category/hull-length/quantity/
                             hull-defect-rate record. `:verified?`
    ...
    - `equipment`         -- a hull-molding/rigging/final-assembly/
                             water-test unit's own record.
    ...
    - `maintenance`       -- a scheduled hull-molding/rigging/final-
                             assembly/water-test maintenance-window
    ...
    - `shipments`         -- a proposed outbound boat shipment DRAFT
                             (`boatmfg.registry`'s `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (boatmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (boatmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :sailboat
                :hull-length-m 12.5
                :quantity-units 40.0 :hull-defect-rate-percent 0.6
                :verified? true :registered? true
                :shipped-units 10.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :personal-watercraft
                :hull-length-m 3.5
                :quantity-units 80.0 :hull-defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :motorboat
                :hull-length-m 8.0
                :quantity-units 15.0 :hull-defect-rate-percent 2.3
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"molder-001" {:id "molder-001" :kind :hull-molding-press
                 :verified? true :registered? true
                 :last-maintenance-date "2026-05-01"}
   "watertest-002" {:id "watertest-002" :kind :water-test-tank
                     :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `boatmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `boatmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- one verified+registered sailboat batch with shipping headroom
  (schedulable), one verified+registered personal-watercraft batch that
  ...
  verified+registered hull-molding-press unit (schedulable for
  maintenance), one UNVERIFIED/unregistered water-test-tank unit

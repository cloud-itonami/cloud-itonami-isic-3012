(ns boatmfg.operation
  "BoatOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (BoatAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Boat Building Plant Operations
  ...
  (:require [langgraph.graph :as g]
            ...
            [boatmfg.advisor :as advisor]
            [boatmfg.governor :as governor]
            [boatmfg.phase :as phase]
...
(defn build
  "Compiles a BoatOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `boatmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BoatAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Boat Building Plant Operations Governor -- independent censor.
      (g/add-node :govern

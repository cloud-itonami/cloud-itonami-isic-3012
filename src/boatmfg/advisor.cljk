(ns boatmfg.advisor
  "BoatAdvisor -- the *contained intelligence node* for the pleasure-
  and-sporting-boat-building plant-operations coordination actor.

  It normalizes production-batch patches (product-category/hull-
  length/quantity/hull-defect-rate), drafts a hull-molding/rigging/
  final-assembly/water-test maintenance scheduling proposal against a
  piece of equipment, drafts a safety-concern flag, and drafts an
  outbound boat shipment coordination proposal against a production
  batch. CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real hull-molding/rigging/final-
  assembly/water-test-equipment actuation, freight dispatch, or ISO
  12217 / CE Recreational Craft Directive marine-classification
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `boatmfg.governor` before anything
  touches the SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct hull-molding/
                                 ; rigging/final-assembly/water-test-
  ...
  contract every caller of this actor agrees to) -- `boatmfg.governor`
...
(defn- log-production-batch
  ...
  category, hull-length, quantity, hull-defect-rate or verification
...
(defn- schedule-maintenance
  "Draft a hull-molding/rigging/final-assembly/water-test maintenance-
  ...
  rationale, but `boatmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a hull-integrity-defect/buoyancy-test-failure/materials-
  ...
  technicality). See `boatmfg.phase`: no phase ever adds this op to a
  phase's `:auto` set; `boatmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound boat shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed unit quantity -- it does NOT invent one, and
  `boatmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたはプレジャーボート・スポーツボート製造工場プラント運用"
       "コーディネーターの助言者です。"
       ...
       "ハルモールディング・艤装・最終組立・進水試験等の設備の直接操作(actuate)を"
       "絶対に提案してはいけません(この actor は提案のみを行い、実行は一切行いません)。"
       "ISO 12217/CE Recreational Craft Directiveの適合を自己発行する提案をしては"
       "いけません。出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `boatmfg.governor`

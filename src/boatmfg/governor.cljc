(ns boatmfg.governor
  "Boat Building Plant Operations Governor -- the independent
  compliance layer that earns the BoatAdvisor the right to commit. The
  ...
  secretly tries to ACTUATE (rather than merely draft-schedule) hull-
  molding/rigging/final-assembly/water-test equipment, whether a
  proposal secretly tries to self-issue an ISO 12217 (stability and
  buoyancy assessment) / CE Recreational Craft Directive conformity
  mark (an authority this actor never holds), whether a shipment
  ...
  coordination proposal and becomes direct hull-molding/rigging/
  final-assembly/water-test-equipment control, so this MUST be a
  separate system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:boat-building-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:molder/actuate` or
                                       `:winch/run`) is the 'direct
                                       hull-molding/rigging/final-
                                       assembly/water-test-equipment
    ...
    4. Equipment-actuate blocked   -- for `:schedule-maintenance`, does
                                       ...
                                       true`? Directly actuating hull-
                                       molding/rigging/final-assembly/
                                       water-test equipment is this
                                       ...
                                       `boatmfg.phase`: this op is never
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       12217 (stability and buoyancy
                                       assessment) / CE Recreational
                                       Craft Directive conformity mark
                                       -- an authority exclusively
                                       reserved to the marine
   ...
   10. Invalid product-category    -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`boatmfg.registry/product-
                                       category-valid?`), the batch
                                       record is rejected rather than
                                       let a fabricated category
                                       through.
   11. Invalid hull-length         -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:hull-length-m` that is not a
                                       physically plausible rating
                                       (`boatmfg.registry/hull-length-
                                       valid?`), the batch record is
                                       rejected rather than let a
                                       fabricated/sensor-error rating
                                       through.
   12. Invalid hull-defect-rate    -- for `:log-production-batch`, if
                                       the patch declares a `:hull-
                                       defect-rate-percent` that is not
                                       a physically plausible reading
                                       (`boatmfg.registry/defect-rate-
                                       valid?`), the batch record is
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [boatmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct hull-molding/
  rigging/final-assembly/water-test-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct hull-molding/rigging/final-assembly/water-
  test-equipment control, a fabricated actuation effect) is this
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はハルモールディング・艤装・最終組立・進水試験等の設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate hull-molding/rigging/final-assembly/water-test
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "ハルモールディング・艤装・最終組立・進水試験等の設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue an ISO 12217 (stability and buoyancy assessment) / CE
  Recreational Craft Directive conformity mark -- an authority
  exclusively reserved to the marine classification society / notified
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "ISO 12217 (安定性・浮力評価) / CE Recreational Craft Directive 適合マークの自己発行提案は恒久的に禁止 -- 船級協会・認証機関の専権事項"}])))
      ...
      (when (and b (registry/shipment-quantity-exceeded? b units))
        [{:rule :shipment-quantity-exceeded
          :detail (str batch-id " の記録済み生産数量(" (:quantity-units b)
                       "隻)を、既存出荷実績(" (:shipped-units b 0.0)
                       "隻)+今回申請(" units "隻)が超過")}]))))
...
(defn- invalid-hull-length-violations
  "For `:log-production-batch`, if the patch declares a
  `:hull-length-m` that is not a physically plausible rating, reject
  rather than let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [hl (:hull-length-m (:value proposal))]
      (when (and (some? hl) (not (registry/hull-length-valid? hl)))
        [{:rule :invalid-hull-length
          :detail (str hl " は物理的に妥当な hull-length-m の範囲外")}]))))

(defn- invalid-defect-rate-violations
  "For `:log-production-batch`, if the patch declares a
  `:hull-defect-rate-percent` that is not a physically plausible
  reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rr (:hull-defect-rate-percent (:value proposal))]
      (when (and (some? rr) (not (registry/defect-rate-valid? rr)))
        [{:rule :invalid-defect-rate
          :detail (str rr "% は物理的に妥当な不良率の範囲外")}]))))

(defn check
  "Censors a BoatAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-hull-length-violations request proposal)

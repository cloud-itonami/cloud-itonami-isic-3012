(ns boatmfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE hull-molding/
  rigging/final-assembly/water-test equipment directly (permanently
  blocked, no override), a proposal that tries to self-issue an ISO
  ...
  implausible hull-length, and a production-batch patch with an
  implausible hull-defect-rate reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on molder-001 (verified, registered, hull-molding press -- escalates, approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:product-category :sailboat}}
                       coordinator))
    ...
    (println "== schedule-maintenance mnt-2 on watertest-002 (UNVERIFIED/unregistered water-test tank -> HARD hold) ==")
    ...
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-category :hovercraft}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible hull-length -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:hull-length-m 9999.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible hull-defect-rate reading -> HARD hold ==")
    (println (exec-op actor "t14"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:hull-defect-rate-percent 999.0}}
                       coordinator))

    (println "== log-production-batch batch-001 attempting to self-issue an ISO 12217/CE RCD certification -> HARD hold, PERMANENT ==")

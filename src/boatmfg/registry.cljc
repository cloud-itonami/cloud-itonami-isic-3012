(ns boatmfg.registry
  "Pure-function domain logic for the pleasure-and-sporting-boat-
  building plant-operations coordination actor -- equipment/batch
  verification, shipment-quantity recompute, product-category
  validation, hull-length plausibility validation, hull-defect-rate
  ...
  pre-existing `kotoba-lang/boatmfg`-style capability library to wrap
  (verified: no such repo exists). The domain logic therefore lives
  here as pure functions, re-verified INDEPENDENTLY by
  `boatmfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (most
  directly `bikemfg.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-3092`): never trust a proposal's own
  ...
  coordinated shipment), not the act of actuating hull-molding/
  rigging/final-assembly/water-test equipment or dispatching a real
  freight carrier, and never the act of issuing an ISO 12217
  (stability and buoyancy assessment) / CE Recreational Craft
  Directive conformity mark (this actor NEVER does any of those -- see
  README `What this actor does NOT do`).

  SCOPE: ISIC 3012 covers building of pleasure and sporting boats --
  hull-molding, rigging, final-assembly, and water-test (buoyancy/
  stability) inspection lines producing finished sailboats, motor
  boats, personal watercraft, canoes, kayaks and other pleasure/
  sporting craft. This actor coordinates the back-office record-
  keeping around that plant (production-batch logging, maintenance
  scheduling, safety-concern flagging, shipment coordination) -- it
  never touches the hull-molding/rigging/final-assembly/water-test
  equipment directly, and it never stands in for the marine
  classification society / notified body that issues seaworthiness
  certification.")
...
(def valid-product-categories
  ...
  may declare -- spans the pleasure-and-sporting-boat families ISIC
  3012 covers (inflatable boats, sailboats, motor boats, canoes,
  kayaks, skiffs and other small craft for sport/pleasure use).
  Anything else is a fabricated/unrecognized product category -- the
  governor HARD-holds rather than let an invented category pass
  through."
  #{:sailboat :motorboat :inflatable-boat :yacht :catamaran
    :personal-watercraft :sport-fishing-boat :canoe :kayak :skiff
    :rowing-boat})

(def hull-length-min-m
  "Physical floor for a boat's own rated hull length overall (LOA)."
  0)

(def hull-length-max-m
  "Physical ceiling for a boat's own rated hull length overall (LOA)
  -- spans small kayaks/skiffs through large pleasure/sporting motor
  yachts and sailing yachts still within ISIC 3012's 'pleasure and
  sporting boats' scope (distinct from ISIC 3011's larger commercial/
  naval ship hulls). A reading above this is an implausible/fabricated
  measurement, not a real standardized hull-length spec."
  100)

(def defect-rate-min-percent
  "Physical floor for a batch's own hull-molding/lamination defect-rate
  reading (zero defects is the best possible outcome, never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own hull-molding/lamination defect-
...
(defn batch-verified?
  ...
  (i.e. its product-category/hull-length/quantity/hull-defect-rate
...
(defn product-category-valid?
  ...
  values (pleasure/sporting boats)? nil/blank is treated as invalid (a
  production-batch patch must declare a real product category, not
  omit it silently)."
  [product-category]
  (contains? valid-product-categories product-category))

(defn hull-length-valid?
  "Is `hull-length-m` a physically plausible hull length overall
  (LOA)? Rejects nil, non-numbers, negative values, and values beyond
  `hull-length-max-m` -- a fabricated or sensor-error reading, never
  let through as a real product-rating fact."
  [hull-length-m]
  (and (number? hull-length-m)
       (>= (double hull-length-m) (double hull-length-min-m))
       (<= (double hull-length-m) (double hull-length-max-m))))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch hull-molding/lamination
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER an ISO 12217 stability-and-buoyancy assessment or
  a CE Recreational Craft Directive conformity mark -- this actor is
  never the marine-classification/seaworthiness-certification
...
(defn register-maintenance
  ...
  hull-molding/rigging/final-assembly/water-test maintenance window
  against a verified, registered piece of equipment. Pure function --
  does not actuate the hull-molding/rigging/final-assembly/water-test
  equipment or execute any maintenance; it builds the RECORD a plant
  coordinator would keep. `boatmfg.governor` independently re-verifies
  the equipment's own verified/registered ground truth, and
  permanently blocks any attempt to directly actuate the hull-molding/
  rigging/final-assembly/water-test equipment (see README
...
(defn register-shipment
  ...
  outbound boat shipment against a verified, registered production
  batch. Pure function -- does not dispatch any real freight carrier;
  it builds the RECORD a plant coordinator would keep. `boatmfg.governor`
  independently re-verifies the shipment's own claimed quantity
  against `shipment-quantity-exceeded?`, before this is ever allowed
  to commit."
